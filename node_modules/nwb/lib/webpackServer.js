...
function getServerOptions(args, cb) {
  ...
  (0, _detectPort2.default)(intendedPort, function (err, suggestedPort) {
    ...
    if (suggestedPort === intendedPort) return cb(null, { fallback: fallback, host: host, port: intendedPort });
    if (args.force) return cb(null, { fallback: fallback, host: host, port: suggestedPort });
    ...
    _inquirer2.default.prompt([{
      ...
      var run = _ref.run;
      return cb(null, run ? { fallback: fallback, host: host, port: suggestedPort } : null);
    }, function (err) {
      return cb(err);
    });
  });
